...
exports.login = (req, res) => {
    User.findByPk(req.body.email).then(user => {         //procurar pelo id do link
        if(user == null) {   
            res.status(401).send("<head><meta http-equiv='refresh' content='2;url=../'/><title>Redirect Page</title></head><body>E-mail ou Senha errada.</body>");
        }else{
...
exports.cadastro = (req, res) => {
    ...
    try{
        User.findByPk(req.body.email).then(user => {         //procurar pelo id do link
            if(user != null) {   
                res.status(401).send("<head><meta http-equiv='refresh' content='2;url=../'/><title>Redirect Page</title></head><body>E-mail já cadastrado.</body>");
            }else{
                if(req.body.password.length >= 8){
                    ...
                    res.status(401).send("<head><meta http-equiv='refresh' content='2;url=../'/><title>Redirect Page</title></head><body>A senha deve conter no mínimo 8 dígitos.</body>");
                }
            }
        });
    }catch(error) {
